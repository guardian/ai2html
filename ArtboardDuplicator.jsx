...
function main() {
  var SCRIPT = {
      ...
      CFG = {
        ...
        spacing: 300, // Default spacing between copies (doc units)
...
function duplicateArtboard(thisAbIdx, items, spacing, suffix, counter) {
  ...
  if (counter === 0) {
    ...
    newAb.artboardRect = [
      lastAbRect[0],
      lastAbRect[3] - spacing,
      lastAbRect[2],
      lastAbRect[3] - spacing - abHeight
    ];
  }
  // if (counter === 0) {
  //   newAb.artboardRect = [
  ...
  for (var i = 0, dLen = dupArr.length; i < dLen; i++) {
    ...
    dupArr[i].position = [pos[0], pos[1] - (abHeight + spacing) * (counter + 1)];
  }
}
